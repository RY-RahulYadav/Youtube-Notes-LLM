...
# --- List of free proxies to try ---
FREE_PROXIES = [
    "http://51.158.68.68:8811",
    "http://185.199.229.156:443",
    "http://51.91.144.100:8080",
    # Add more free proxies if needed
]
...
def fetch_transcript_with_proxies(video_id: str):
    last_exception = None
    for proxy in FREE_PROXIES:
        try:
            transcript_list = YouTubeTranscriptApi.list_transcripts(video_id, proxies={"http": proxy, "https": proxy})
            try:
                transcript = transcript_list.find_transcript(['en'])
            except Exception:
                first_transcript = list(transcript_list)[0]
...
def fetch_transcript_in_english(video_url: str) -> str:
    try:
        ...
        try:
            transcript_text = fetch_transcript_with_proxies(video_id)
        except Exception as e:
